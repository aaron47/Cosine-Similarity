...
class AllMpnetBaseV2:
    ...
    def perform_cosine_similarity_between_2_sentences(self, sentences: list[str] | None = None) -> Number:
        ...
        same_dim_sentence_embeddings = [sentence.unsqueeze(0) for sentence in sentence_embeddings]

        # Perofrm cosine similarity
        similarity = F.cosine_similarity(same_dim_sentence_embeddings[0], same_dim_sentence_embeddings[1])
    ...
    """
    def perform_cosine_similarity_and_return_highest(self, sentences: list[str]) -> tuple[Number, tuple[str | None, str | None]]:
        sentence_embeddings = self.__encode_sentences_and_normalise(sentences)
        ...
        for i in range(num_sentences):
            for j in range(i+1, num_sentences): # Compare each pair only once
                ...
                similarity = F.cosine_similarity(sentence_embeddings[i].unsqueeze(0), sentence_embeddings[j].unsqueeze(0)).item() 
    ...
    def encode_sentence_and_normalise(self, sentence: str) -> torch.Tensor:
        ...
        # Perform Pooling
        sentence_embedding = self.__mean_pooling(model_output, attention_mask=encoded_input["attention_mask"])
    ...
    """
    def __mean_pooling(self, model_output: torch.Tensor, attention_mask) -> torch.Tensor:
    ...
    """
    def __encode_sentences_and_normalise(self, sentences: list[str] | None) -> torch.Tensor:
        # Tokenize sentences
        ...
        # Perform Pooling
        sentence_embeddings = self.__mean_pooling(model_output=model_output, attention_mask=encoded_input["attention_mask"])
...
if __name__ == "__main__":
    ...
    diff1 = model.perform_cosine_similarity_between_2_sentences(sentences=["This is a sentence", "This is another sentence"])
    diff2 = model.perform_cosine_similarity_and_return_highest(sentences=["C'est une premiere phrase en francais", "C'est une autre", "Une autre", "N'importe quoi"])
    sentence_embedding = model.encode_sentence_and_normalise("lolll this is a sentence")
    ...
    """
        ...
        4 SENTENCES
        (0.8562366962432861, ("C'est une autre", 'Une autre'))
    """
    print("2 SENTENCES")
    print(diff1)
    ...
    # Should print the sentence in the form of an encoded tensor
    print("EMBEDDING")
